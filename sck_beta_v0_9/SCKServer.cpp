...
void SCKServer::updateThingSpeak(String tsData)
{
    Serial.println(F("TS1"));
    if (base__.enterCommandMode()) 
    {Serial.println(F(" TS2"));
      if (base__.open(WEBTS[0], 3000))
      //if (base__.open(WEBTS[0], 80))
      { 
	    Serial.println(F(" TS3 "));
		Serial1.print("POST /update.json HTTP/1.1\n");
		Serial1.print("Host: api.thingspeak.com\n");
		Serial1.print("Connection: close\n");
//		Serial1.print("X-THINGSPEAKAPIKEY: "+WEBTS[1]+"\n");
		Serial1.print("X-THINGSPEAKAPIKEY: ");
		Serial1.print(WEBTS[1]);
		Serial1.print("\n");
		Serial1.print("Content-Type: application/x-www-form-urlencoded\n");
		Serial1.print("Content-Length: ");
		Serial1.print(tsData.length());
		Serial1.print("\n\n");

		Serial1.print(tsData);
      ...
      if (base__.connect())  //Wifi connect
        {
		  ...
		  //updateThingSpeak("field1="+(String)value[0] );
                Serial.print(F("V1:"));
            ...
            updateThingSpeak("field1="+(String)value[0]+"&field2="+(String)value[1]+"&field3="+(String)value[2]+"&field4="+(String)value[3]+"&field5="+(String)value[4]+"&field6="+(String)value[5]+"&field7="+(String)value[6]+"&field8="+(String)value[7]);
            Serial.print(F("V2:"));
            Serial.println((String)value[0]);
            #if debugEnabled
